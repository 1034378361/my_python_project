...
class TestVersionIntegration:
    ...
    def test_package_metadata_version(self):
        """测试包元数据版本"""
        # 验证包元数据中的版本信息
        expected_metadata_fields = ["version", "name", "description", "author"]
...
class TestVersionSchemes:
    ...
    def test_no_local_version_scheme(self):
        ...
        invalid_local_versions = ["1.0.0+local", "1.0.0+abc123", "1.0.0+dirty"]
...
class TestVersionUtilities:
    ...
    def test_version_parsing(self):
        """测试版本解析"""
        test_versions = ["1.0.0", "1.2.3", "0.1.0", "10.20.30"]
...
class TestVersionBuildIntegration:
    ...
    def test_git_tag_version_generation(self):
        """测试Git标签版本生成"""
        # 模拟Git标签版本生成
        mock_git_tags = ["v1.0.0", "v1.1.0", "v1.2.0", "v2.0.0"]
