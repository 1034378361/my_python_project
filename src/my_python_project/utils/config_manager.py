...
try:
    ...
    try:
        ...
        HAS_TOML = False
        warnings.warn(
            "tomllib/tomli not installed, TOML support disabled", stacklevel=2
        )
...
class ConfigManager:
    ...
    @classmethod
    def from_dict(
        cls, config_dict: Dict[str, Any], env_prefix: Optional[str] = None
    ) -> "ConfigManager":
        """从字典创建配置管理器"""
            ...
            for key, expected_type in schema_data.items():
                ...
                if isinstance(expected_type, dict):
                    ...
                    if not isinstance(value, expected_type):
                        errors.append(
                            f"配置项 {full_path} 应该是 {expected_type.__name__} 类型"
                        )
    ...
    def create_backup(self, backup_dir: Optional[Union[str, Path]] = None) -> Path:
        ...
        backup_dir = (
            Path(backup_dir) if backup_dir else self.config_path.parent / "backups"
        )
...
def get_config(
    key: str, default: Optional[T] = None, type_hint: Optional[Type[T]] = None
) -> T:
    """快捷方式获取全局配置值"""
