...
class NumberValidator(Validator):
    ...
    def _validate_value(
        ...
        if self.min_value is not None and value < self.min_value:
            raise ValidationError(f"字段 '{field_name}' 不能小于 {self.min_value}")

        if self.max_value is not None and value > self.max_value:
            raise ValidationError(f"字段 '{field_name}' 不能大于 {self.max_value}")
...
class ListValidator(Validator):
    ...
    def _validate_value(self, value: Any, field_name: str) -> List[Any]:
        ...
        # 长度验证
        if self.min_length is not None and len(value) < self.min_length:
            raise ValidationError(f"字段 '{field_name}' 长度不能少于 {self.min_length}")

        if self.max_length is not None and len(value) > self.max_length:
            raise ValidationError(f"字段 '{field_name}' 长度不能超过 {self.max_length}")
...
class DateValidator(Validator):
    ...
    def _validate_value(self, value: Any, field_name: str) -> date:
        ...
        if self.min_date and date_value < self.min_date:
            raise ValidationError(f"字段 '{field_name}' 不能早于 {self.min_date}")

        if self.max_date and date_value > self.max_date:
            raise ValidationError(f"字段 '{field_name}' 不能晚于 {self.max_date}")
            ...
            for field_name, validator in field_validators.items():
                if field_name in kwargs:
                    try:
                        ...
                        raise ValidationError(f"函数 {func.__name__} 参数验证失败: {e}")
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                raise ValidationError(f"函数 {func.__name__} 返回值验证失败: {e}")
...
def create_validator_from_type(type_hint: Type) -> Validator:
    ...
    if origin is list:
        ...
        item_validator = (
            create_validator_from_type(item_type) if item_type != Any else None
        )
