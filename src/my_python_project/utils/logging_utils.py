...
def get_project_logger(
    module_name: str = None, log_file: Union[str, Path] = None
) -> logging.Logger:
    """获取项目专用的日志记录器。
...
def create_rotating_log(
    log_dir: Union[str, Path], name: str, **kwargs
) -> logging.Logger:
    """创建轮转日志（向后兼容）。"""
    log_file = Path(log_dir) / f"{name}.log"
    return setup_advanced_logger(
        name=name, log_file=log_file, enable_rotation=True, **kwargs
    )
...
def get_logger_with_path(
    log_path: Union[str, Path], module_name: str = None
) -> logging.Logger:
    """获取指定路径的项目日志记录器（便捷函数）。
...
def setup_logging_with_path_manager(
    ...
    **logger_kwargs,
    ...
    return setup_advanced_logger(
        name=f"my_python_project.{module_name}", log_file=log_path, **logger_kwargs
    )


def configure_project_logging_with_paths(
    path_manager, log_configs: Optional[Dict[str, Dict]] = None, **base_config
) -> Dict[str, logging.Logger]:
    """
    使用路径管理器配置完整的项目日志系统
    ...
    for logger_name, config in log_configs.items():
        ...
        full_logger_name = f"my_python_project.{logger_name}"
        loggers[logger_name] = setup_advanced_logger(
            name=full_logger_name, log_file=log_path, **merged_config
        )
...
def auto_setup_project_logging(
    ...
    **path_manager_kwargs,
    ...
    path_manager = init_project_paths(
        base_dir=base_dir, session_name=session_name, **path_manager_kwargs
    )
...
def load_logging_config_from_file(
    config_file: Union[str, Path] = None, config_format: str = "auto"
) -> bool:
    """从配置文件加载日志配置。
