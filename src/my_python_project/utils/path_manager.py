...
class ProjectPathManager:
    ...
    def __str__(self) -> str:
        """字符串表示"""
        return (
            f"ProjectPathManager(session='{self.session_name}', base='{self.base_dir}')"
        )

    def __repr__(self) -> str:
        """详细字符串表示"""
        return (
            f"ProjectPathManager(base_dir='{self.base_dir}', "
...
def init_project_paths(
    base_dir: Union[str, Path], session_name: Optional[str] = None, **kwargs
) -> ProjectPathManager:
    """
    初始化全局项目路径管理器
